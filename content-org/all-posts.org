...
* TODO Using the Picarro G2201-i with SSIM2 module
:PROPERTIES:
...
#+attr_shortcode: Warning
#+begin_alert2
*This instrument is expensive.* The Picarro G2201-i analyzer costs approximately $200,000. The SSIM2 module adds another $15,000-20,000. Each vacuum pump is $2,500. Handle with care, follow all procedures precisely, and never force connections or ignore error messages.
#+end_alert2

#+attr_shortcode: Warning
#+begin_alert2
*The vacuum pump must be running when the analyzer is powered on.* The Picarro's internal cavity operates at reduced pressure. If the vacuum pump is off while the analyzer is on, pressure will build up inside the cavity, potentially damaging sensitive optical components. Treat the vacuum pump as part of the analyzer's power system. This is not optional.
#+end_alert2

** Introduction

The Picarro G2201-i is an isotopic analyzer that measures both CO₂ and CH₄ concentrations along with their stable carbon isotope ratios (δ¹³C-CO₂ and δ¹³C-CH₄). The instrument uses Cavity Ring-Down Spectroscopy (CRDS), a laser-based technique that provides part-per-billion sensitivity and very good isotopic precision.

The Small Sample Isotope Module (SSIM, or SSIM2: the improved model) is an accessory that enables analysis of discrete gas samples as small as ~20 ml. Without the SSIM, the Picarro operates in continuous-flow mode requiring sample volumes exceeding 100 ml. With the SSIM, you can analyze gas from sample bags, vials, soil chamber headspace or any small container, which makes it great for sampling limited-run or small-volume samples.
...
5. *Sample containers* with manual valves. These can be Tedlar bags, glass flasks, syringes, or any container that can be attached via 1/8" Swagelok fittings and has a manual valve to isolate it during purging.
...
6. *1/8" stainless steel tubing and Swagelok fittings* (various lengths). All gas connections use 1/8" Swagelok compression fittings.

7. *Wrenches for Swagelok fittings*: 7/16", 1/2", 9/16", and 11/16" inch wrenches.
...
5. *Connect calibration gas (optional):*
...
**** Installing the SSIM Coordinator
...
**** Analyzer Bootloop (RAM Reseating)

*Symptom:* The analyzer repeatedly restarts without fully initializing. Green ring light comes on for a few seconds, fans audibly spin up, but there is no display output and the analyzer powers off after 3 seconds. This cycle repeats indefinitely.

*Cause:* Dust or poor electrical contact in the RAM slots, often after transport or storage.
...
**** "Pressure High" or "Pressure Low" Warnings (Normal)
...
**** Sample Reads as Ambient Air (~420 ppm CO₂, δ¹³C ≈ -8‰)
...
**** High Variability Across Replicates (SD > 0.5‰)
...
**** Values Don't Stabilize (CO₂ Continues Drifting)
...
**** SSIM Pressure Won't Drop Below 50 Torr During Evacuation
...
**** "Error: Could Not Find Valve" in Coordinator Window
...
*** Cleaning the SSIM Sample Chamber
...
** Appendix: Gas Cylinder Specifications
...
** Appendix: Software Files and Paths
...
5. Save the file
6. The new reference gas will appear in the dropdown when you next launch the Coordinator
...
If you encounter issues not covered in this guide, consult the official Picarro G2201-i manual, contact Picarro support, or reach out to [[mailto:morgan@mor-gan.com][morgan@mor-gan.com]].
