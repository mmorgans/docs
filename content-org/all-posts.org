...
$$\frac{dC}{dt} = \frac{C_{final} - C_{initial}}{\Delta t} = \frac{450 - 400}{90} = 0.556 \text{ ppm s}^{-1}$$
...
Where:
- $C_{dry}$ = CO₂ mole fraction in dry air (ppm)
- $C_{wet}$ = measured CO₂ in humid air (ppm)
