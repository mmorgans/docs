...
The Field Portable Gas Analyzer (FPGA) draws soil gas to the surface, measures its CO_{2} and O_{2} concentrations, and can collect a sample for later laboratory analysis. The design comes from [[https://doi.org/10.1371/journal.pone.0220176][Brecheisen et al. (2019)]], which describes a set of commercially available sensors and plumbing assembled into a battery-powered instrument small enough to carry between wells.

Inside the Pelican case are a Vaisala ~GMP221~ CO_{2} probe and ~GMM220~ transmitter, an Apogee ~MO220~ O_{2} meter, a diaphragm pump, a water trap, a vacuum gauge, and six three-way valves. The valves let you switch between measuring gas from a well, accepting a sample from a syringe or bag, or reversing the flow through the trap to clear water.

#+attr_shortcode: Note
#+begin_alert2
*YMMV.* Moreso in this article than in others. You should confirm the specifics of your situation as they may differ from those described below. As usual, I'll do my best to help if you [[mailto:fpga@mor-gan.com][send me an email]].
#+end_alert2

** What the FPGA measures

The Apogee meter displays O_{2} concentration directly as a percentage. The Vaisala measures CO_{2} by infrared absorption, but this build presents its output as a voltage on the multimeter. You'll need to convert that voltage to concentration.
...
4. Check that O_{2} is close to ambient, approximately 20.95% last time I checked.
5. Record ambient CO_{2} in mV and converted concentration, O_{2} in percent, and temperature. Be sure to direct your breath away from the FPGA.
...
4. Turn the pump off. Leave valve 6 at the white return and observe both gas readings for about a minute. Record the CO_{2} peak and the O_{2} sample reading, following the measurement-window note below.

Brecheisen et al. note that CO_{2} keeps climbing for a bit after the pump stops, because the probe's protective sleeve slows it down. The reading usually peaks and then drifts back. Record the peak.

#+attr_shortcode: Note
#+begin_alert2
*Keep recirculation brief.* The paper saw CO_{2} drop by up to 0.1–0.2 percentage points when gas circulated, and prefers the non-circulating method when there's enough sample. Write down which method you used.
#+end_alert2

Sample from low to high expected CO_{2} where you can (usually shallow to deep), and flush between wells to limit carryover.

#+attr_shortcode: Note
#+begin_alert2
*Which O_{2} reading?* The paper says to record the maximum of both gases within a minute. That works for CO_{2}, but in an oxygen-depleted sample the O_{2} maximum is often just the ambient reading the meter started at. Take O_{2} once the sample has arrived and the number has settled, and pair it with the CO_{2} from the same gas. If it never settles, write down the range.
#+end_alert2
...
2. Turn valve 4 to the collection port (OFF handle toward the CO_{2} sensor).
...
5. Record the CO_{2} peak and the settled O_{2} reading.
...
Near ambient, CO_{2} may not show a clear peak, and small differences are hard to see. For small incubation headspaces, make sure you have enough gas before you start.
...
Stop the pump before disconnecting a well. Put the valves back to their starting routes with valve 6 on free air, and draw room air through the black intake until both readings return to your ambient baseline. Don't move on until they do, especially after a high-CO_{2} sample or a one-port read.
...
- Ambient CO_{2} and O_{2}, temperature, and any compensation you applied.
...
If you use the same multimeter for temperature, record the CO_{2} voltage first, then switch the leads over to the thermocouple. Switch them back before the next sample.
...
Check the meter's battery and the gas path, and give it more time in clean air. If it's still off, calibrate the meter per its manual and check it against a certified standard. If you're working at low O_{2}, check a low standard too.
...
#+attr_shortcode: Caution
#+begin_alert2
*Don't use CO_{2} + O_{2} ≈ 21% as a leak test.* Good samples can land there too. Check the connections and run a standard instead.
#+end_alert2
...
Brecheisen et al. calculate the apparent respiratory quotient from the rise in soil CO_{2} above ambient and the drop in O_{2} below ambient, multiplied by 0.76 to account for the two gases' different diffusivities:

$$\mathrm{ARQ} = 0.76\,\frac{C_{\mathrm{soil}}-C_{\mathrm{ambient}}}{O_{\mathrm{ambient}}-O_{\mathrm{soil}}}$$

$C$ is CO_{2} and $O$ is O_{2}, all in the same units; percent is easiest. The denominator is *ambient minus soil*, so it comes out positive.

For example, with ambient air at 0.04% CO_{2} and 20.95% O_{2}, and soil gas at 2.04% CO_{2} and 18.95% O_{2}, both changes are 2.00 percentage points and ARQ = 0.76. The plain ratio would be 1.00.

ARQ gets noisy fast when either change is small, especially when O_{2} barely drops. If the O_{2} change is within your measurement error, skip the ARQ for that sample. Round to the precision your measurements support.
...
Brecheisen et al. compared 1,639 field CO_{2} measurements against gas chromatography across roughly 0.25–7.75% CO_{2}, with a slope of 1.017, an intercept of −0.033, and R^{2} = 0.956. That's good agreement for enriched soil gas. Near ambient, check it yourself with standards.
...
The valve numbering and workflows come from the Kirk lab's working protocol, /FieldPortableGasAnalyzerProtocols_edited.docx/. Its notes leave the voltage scaling and the pump-off readings open, so check those with the instrument's maintainers and a standard gas. Keep the configuration and calibration records in the case.
