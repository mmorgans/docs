...
#+attr_shortcode: Warning
#+begin_alert2
*Keep water away from the sensors.* Water in the trap, condensation in the lines, or a sudden vacuum drop that suggests a flooded intake all mean the same thing: stop the pump. The CO_{2} probe reads badly for a long time after it gets wet, and replacing it is neither fast nor cheap.
#+end_alert2
...
The Field Portable Gas Analyzer is a home-built field rig for pulling soil gas up out of a well, measuring O_{2} and CO_{2}, and collecting a sample if you want one for the lab later. It came out of [[https://doi.org/10.1371/journal.pone.0220176][Brecheisen et al. 2019]], which describes the design and validates it against gas chromatography. That paper is worth reading if you want to know why the thing is built the way it is; this guide is about operating it.

The parts that matter: a Vaisala ~GMP221~ CO_{2} probe on a ~GMM220~ transmitter module, an Apogee ~MO220~ O_{2} meter, a diaphragm pump, a water trap, a vacuum gauge, and six numbered three-way valves that decide where the gas goes. All of it lives in a Pelican case and runs off a 12 V battery. The paper puts the build at roughly $2,000 and under 10 kg including the battery, which is the entire point of it existing.
...
O_{2} comes off the Apogee meter directly, in percent. CO_{2} does not: the Vaisala module puts out an analog voltage, and you read that voltage on the multimeter and convert it yourself. Temperature is separate, on the thermocouple in the case.

Measuring both gases at once is the reason to bother with this instrument rather than a CO_{2}-only analyzer. Soil CO_{2} usually climbs while O_{2} falls, and the ratio of those two changes carries information that CO_{2} alone doesn't — about what's being respired, how well the pores are connected, and how anaerobic things have gotten down there. That ratio is the Apparent Respiratory Quotient. See [[Appendix: ARQ][the ARQ appendix]] if you plan to calculate it, because there are a few ways to get it wrong.
...
Once you do, it's a straight proportion. For a unit configured 0–1 V across 0–1% CO_{2}:
...
| 4 | Pump toward the CO_{2} sensor | Opens the collection port for filling a bag or syringe |
| 5 | O_{2} sensor to the check valve and outlet | Left alone during routine sampling |
| 6 | Where gas exits after the sensors | Free air, the white return port, or sealed for one-port mode |

Gas otherwise travels one direction: black intake, water trap, pressure gauge, pump, CO_{2} sensor, O_{2} sensor, check valve, out. The check valve stops room air backing into the outlet when the pump is off.
...
*** Four numbers to measure once and write on the case
...
- *How long the CO_{2} probe takes to settle from cold.* You need this every time you power it up.
...
2. Power on the CO_{2} sensor, the O_{2} meter, and the pump, with nothing connected to the intake or the return port.
3. Let it run on room air for about a minute.
4. Confirm O_{2} reads close to ambient — around 20.95%. If it doesn't, stop and sort that out before sampling.
5. Read the CO_{2} baseline on the multimeter and *write it down*. Outdoor air is the number you want; indoor air runs higher and varies with how many people are standing around.
6. Look at the water trap and the vacuum gauge before you attach anything.

That ambient CO_{2} and O_{2} pair is not a formality — every delta you calculate later comes off it, measured at that site on that day. Textbook atmospheric values are not a substitute.
...
6. Shut the pump off and record the highest CO_{2} and O_{2} you see over the next minute or so.

CO_{2} will rise to a peak and then sag slightly. That's the probe settling, not the soil gas changing — record the peak. Recording the lower, calmer-looking number biases your whole dataset in a direction you won't notice until much later.

#+attr_shortcode: Note
#+begin_alert2
*Keep the loop circulating for as short a time as you can.* The paper reports an apparent dilution of 0.1–0.2% CO_{2} from this recirculating method. Closing the loop is how the workflow works; leaving it closed longer than necessary is not.
#+end_alert2

If you're working a series of wells, go from low CO_{2} to high where you can — usually shallow before deep. It cuts down on carryover.
...
2. Turn valve 4 to close the path to the CO_{2} sensor and open the collection port.
3. Run the pump and fill the container.
4. Return valve 4 to the sensor path before you disconnect anything.

Label it immediately — well, depth, date, time. Bags are a transport container, not a storage container; if the sample isn't getting analyzed promptly, or you care about the O_{2} number, move it into an evacuated vial.
...
6. Record the peak CO_{2} and O_{2} over the next minute.
...
6. Record the peak CO_{2} and O_{2}.
...
1. Pump off, then the CO_{2} sensor, then the O_{2} meter.
...
*** Everything reads plausibly but nothing agrees between wells

Carryover is the usual culprit. Flush longer between samples, and work low concentration to high.

** Appendix: ARQ

The Apparent Respiratory Quotient is the ratio of how much CO_{2} appeared to how much O_{2} disappeared:

$$\mathrm{ARQ} = \frac{\Delta \mathrm{CO_2}}{\Delta \mathrm{O_2}}$$
...
Interpreting the number once you have a good one — what pushes it above or below 1, and how much of that is biology versus the different diffusion rates of CO_{2} and O_{2} through wet pores — is a real literature and out of scope here.
...
Brecheisen et al. validated CO_{2} against gas chromatography across 1,639 paired measurements: slope 1.017, intercept −0.033, R^{2} 0.956. Precision is given as at least 0.1% for both gases, and again that figure is absolute. At 5% CO_{2} that's a couple percent of the reading. At 0.2% CO_{2} it's half of it.
