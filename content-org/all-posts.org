...
Brecheisen et al. note that CO_{2} keeps climbing for a bit after the pump stops, because the probe's protective sleeve slows it down. Record the peak.

#+attr_shortcode: Note
#+begin_alert2
*Keep recirculation brief.* The paper saw CO_{2} drop by up to 0.1–0.2 percentage points when gas circulated, and prefers the non-circulating method when there's enough sample.
#+end_alert2

Sample from low to high expected CO_{2} where you can, usually shallow to deep.

#+attr_shortcode: Note
#+begin_alert2
*Which O_{2} reading?* The paper says to record the maximum of both gases within a minute. That works for CO_{2}, but in an oxygen-depleted sample the O_{2} maximum is often just the ambient reading the meter started at. Take O_{2} once the number has settled.
#+end_alert2
...
Near ambient, CO_{2} may not show a clear peak.
...
#+attr_shortcode: Caution
#+begin_alert2
*Don't use CO_{2} + O_{2} ≈ 21% as a leak test.* Good samples can land there too.
#+end_alert2
...
$C$ is CO_{2} and $O$ is O_{2}, all in the same units.
...
ARQ gets noisy fast when either change is small. If the O_{2} change is within your measurement error, skip the ARQ for that sample.
...
Brecheisen et al. compared 1,639 field CO_{2} measurements against gas chromatography across roughly 0.25–7.75% CO_{2}, with a slope of 1.017, an intercept of −0.033, and R^{2} = 0.956. That's good agreement for enriched soil gas.
...
The valve numbering and workflows come from the Kirk lab's working protocol, /FieldPortableGasAnalyzerProtocols_edited.docx/. Its notes leave the voltage scaling and the pump-off readings unresolved.

---

/Created in Doom Emacs with Org Mode./

/Written from March to September 2026. Completed in Lui Che Woo Law Library, at the University of Hong Kong./

/Thanks to Louna, who brought me tea as I was writing this./
