...
Although Luke boots to a bootloader offering both Windows 7 and Windows XP, use of Windows 7 is required. Windows XP does not contain the correct version of the Picarro driver and Coordinator and will result in incorrect data.

On the front of Luke is one USB-A port for an accessory, and the power button. On the back left is the standard array of motherboard I/O, the power supply unit switch, and on the back right are the plumbing connections for the analyzer.
...
The opposite is true: zero air is only needed when the SSIM is actively being run, and **it is _bad_ for the analyzer to be fed zero air for extended periods**. When not using the SSIM, disconnect the Swagelok tubing connecting the SSIM to the analyzer, and leave the analyzer inlet open to ambient air.
{{< /alert2 >}}
...
{{< alert2 Note >}}
In a moment of desperation, I became convinced that the length of stainless-steel tubing was causing issues with the outputted data. I commissioned a replacement from Swagelok, which is the currently used part; the lab now has two pieces of identical tubing. Leia, the G2101-i, _also_ has an associated run of stainless-steel tubing for her SSIM, although it's a different length.

I don't believe that the new length of tubing helped the issue. However, it is worth noting that each connection and disconnection of the tubing wears down the ferrule, so the old length of tubing is likely somewhat degraded.
{{< /alert2 >}}
...
4.  Confirm the analyzer-SSIM tubing is connected, if you're using the SSIM.
5.  Open the zero-air cylinder slowly and set delivery to 2–3 psig, if you're using the SSIM.
...
I typically have the following streams on screen:
...
1.  Start with Luke off. Confirm the analyzer vacuum hose is connected, then start the Picarro Analyzer pump and the SSIM pump. Open the tank of zero air, set the regulator to 2–3 psig, ensure it's connected to the SSIM and open the outlet valve. Turn on Luke's rear switch and press the front power button.
...
5.  In Data Viewer, put up `SSIMPressure`, `12CO2` or `12CO2_dry`, your delta field, and either `CavityPressure` or `H2O`. If `SSIMPressure` isn't available, switch Data Viewer to Service mode.
6.  Fill a Luer-lock syringe with lab air and close its stopcock. Keep the unused `CAL` port on the SSIM capped.
7.  Open `SSIM Coordinator`, select `SSIM G2201-i`, enter `2 1 0 0 2 1 2 3 1`. Leave the existing reference-gas entry selected.
...
If you do end up connecting it to the WAN, the typical sanguine advice about Surfing the Net applies. Use Chrome, it's more up to date on Windows 7 than Firefox, and don't be dumb.
